...
""".format(
    ...
    def download_and_unpack_dsc(self, dsc_url, orig_tar_url, debian_tar_url, build_dir):
        logging.info(f"Starting download_and_unpack_dsc with dsc_url: {dsc_url}, orig_tar_url: {orig_tar_url}, debian_tar_url: {debian_tar_url} and build_dir: {build_dir}")

        # Ensure the build directory exists
        if not os.path.exists(build_dir):
            os.makedirs(build_dir, exist_ok=True)
        logging.info(f"Build directory {build_dir} is prepared")

        dsc_file = os.path.join(build_dir, os.path.basename(dsc_url))
        orig_tar_file = os.path.join(build_dir, os.path.basename(orig_tar_url))
        debian_tar_file = os.path.join(build_dir, os.path.basename(debian_tar_url))

        # Download the files
        self.download_file(dsc_url, dsc_file)
        self.download_file(orig_tar_url, orig_tar_file)
        self.download_file(debian_tar_url, debian_tar_file)

        # Unpack the source using dpkg-source
        unpack_cmd = [
            "dpkg-source",
            "--no-check",
            "-x",
        ...
        logging.info(f"Running unpack command: {' '.join(unpack_cmd)}")
        result = subprocess.run(unpack_cmd, cwd=build_dir, capture_output=True, text=True)

        if result.returncode != 0:
            logging.error(f"Failed to unpack source: {result.stderr}")
            raise RebuilderException(f"Failed to unpack source: {result.stderr}")

        # Identify the unpacked directory
        unpacked_dir = next(os.path.join(build_dir, d) for d in os.listdir(build_dir) if os.path.isdir(os.path.join(build_dir, d)))
        logging.info(f"Source code downloaded and unpacked successfully into {unpacked_dir}.")
    ...
    def download_file(self, url, filepath):
        ...
        if response.status_code != 200:
            raise RebuilderException(f"Failed to download file from {url}: HTTP {response.status_code}")
        with open(filepath, 'wb') as f:
            f.write(response.content)
        logging.info(f"File downloaded successfully and saved to {filepath}")
    ...
    def generate_mmdebstrap_cmd(self, output):
        ...
        # Sync-out the built packages
        cmd += [
            '--customize-hook=sync-out /build/src_dir {}'.format(output),
